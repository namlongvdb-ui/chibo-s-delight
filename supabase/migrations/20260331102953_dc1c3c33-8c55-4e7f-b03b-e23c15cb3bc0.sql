...
CREATE TABLE public.digital_signatures (
  ...
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  CONSTRAINT digital_signatures_user_id_key UNIQUE (user_id)
);
...
CREATE POLICY "Admins can delete signatures" ON public.digital_signatures FOR DELETE TO authenticated USING (public.has_role(auth.uid(), 'admin'::public.app_role));
