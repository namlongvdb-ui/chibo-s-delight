...
export function exportStaffListExcel() {
  ...
  const rows: (string | number)[][] = [
    ...
    [],
    ['STT', 'Họ và tên', 'Chức vụ', 'Đơn vị', 'Ngày sinh', 'GT', 'HS lương', 'HS CV', 'Lương KV', 'Lương tính ĐP', 'Đảng phí'],
  ];
  ...
  const ws = XLSX.utils.aoa_to_sheet(rows);
  ws['!cols'] = [
    { wch: 5 }, { wch: 25 }, { wch: 18 }, { wch: 20 }, { wch: 14 }, { wch: 5 },
    { wch: 10 }, { wch: 10 }, { wch: 16 }, { wch: 16 }, { wch: 16 },
  ];
  XLSX.utils.book_append_sheet(wb, ws, 'Danh sách đảng viên');
